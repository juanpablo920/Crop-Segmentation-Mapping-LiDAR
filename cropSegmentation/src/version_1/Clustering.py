...
#     Cluster.cluster()
    ...
    def read_data_clustering(self):
        ...
        data_clustering = data_clustering.drop(['Classification'], axis=1)

        self.pcdcluster = data_clustering.to_numpy()
        print(self.pcdcluster)

        print("datos:", self.pcdcluster.shape)
    ...
    def cluster(self):
        ...
        for k in range(2, 50):
            print("K: ", k)
            kmeans = KMeans(n_clusters=k).fit(self.pcdcluster)
            labels = kmeans.labels_
            print(labels)
            silueta.append(silhouette_score(
                self.pcdcluster, labels, metric='euclidean'))
            print(silueta)
        
        
        max_silueta = np.amax(silueta)
        posMax_silueta = np.where(silueta == max_silueta) 
